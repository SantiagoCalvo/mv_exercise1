...
def sobel(img: np.array) -> (np.array, np.array):
    ...
    gradient_x = cv2.filter2D(src=img, ddepth=cv2.CV_32F, kernel=g_x)
    gradient_y = cv2.filter2D(src=img, ddepth=cv2.CV_32F, kernel=g_y)

    gradient = np.sqrt(gradient_x**2 + gradient_y**2)
    orientation = np.atan2(gradient_y, gradient_x)
